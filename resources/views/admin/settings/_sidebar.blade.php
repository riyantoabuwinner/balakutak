    ...
    <div class="card-body p-0">
        <ul class="nav nav-pills flex-column">
            @foreach($groups as $k => $label)
                <li class="nav-item">
                    <a href="{{ $k == 'infographics' ? route('admin.infographics.index') : route('admin.settings.group', $k) }}" class="nav-link {{ ($group ?? '') == $k || (request()->routeIs('admin.infographics.*') && $k == 'infographics') ? 'active' : '' }}">
